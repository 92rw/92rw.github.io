...
function showWelcome() {
  ...
  try {
    document.getElementById(
      "welcome-info"
    ).innerHTML = `<b><center>🎉 欢迎信息 🎉</center>&emsp;&emsp;欢迎来自 <span style="color:var(--blue-custom)">${ipInfo.location}</span> 的小伙伴，${timeChange}您当前 IP：<span style="color:var(--blue-custom)">${ipInfo.query}</span>，现在距离站长约 <span style="color:var(--blue-custom)">${ipInfo.distance}</span> 公里。${ipInfo.posdesc}</b>`;
  } catch (e) {
    document.getElementById(
      "welcome-info"
    ).innerHTML = `<b><center>🎉 欢迎信息 🎉</center>&emsp;&emsp;欢迎到访本网站，${timeChange}希望您能发现感兴趣的内容</b>`;
...
function createtime1() {
  var grt = new Date("08/25/2021 00:00:00"); //此处修改你的建站时间或者网站上线时间
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| 92rw🚆")
...
if (m == 11 && dd == 11) {
  ...
  if (sessionStorage.getItem("isPopupWindow") != "1") {
    Swal.fire(
      "第一次世界大战结束" + (y - 1918).toString() + "年，唯愿和平永存💐"
    );
    sessionStorage.setItem("isPopupWindow", "1");
  }
}
l = [
  "Pokemon GO也是因愚人节的玩笑而成真的游戏呢！",
  "你的电脑已经过载，建议立即关机！",
  "你知道吗？站长很喜欢你哦！",
  "一分钟有61秒哦",
  "阁下何不乘风起，扶“谣”直上九万里",
...
function createtime() {
  ...
  // 网站诞生时间
  var grt = new Date("08/25/2021 00:00:00");
...
<details class="folding-tag" cyan><summary> 设置自定义背景 </summary>
              <div class='content'>
              <p><center><input type="text" id="pic-link" size="70%" maxlength="1000" placeholder="请输入有效的图片链接，如 https://cn.bing.com/th?id=OHR.AnivaLighthouse_EN-US8147045989_UHD.jpg"></center></p><p><center><button type="button" onclick="getPicture()" style="background:var(--theme-color);width:35%;padding: 5px 0px 7px 0px;border-radius:30px;color:white;line-height:2;">🌈切换背景🌈</button></center></p>
              </div>
            </details>
